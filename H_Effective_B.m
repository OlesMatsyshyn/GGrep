...
xi = 1;
H_eff_B   = zeros (nbands, nbands, nx, ny, q_size);
for iq = 1 : length(qx)
    eig_disr = 0;
    ...
    for i = 1 : nx % to stay in the limit
        for j = 1 : ny
            [Vec,Val] = eigenshuffle(conj(H_eff(xi,-kx(i)+qx(iq)/2,-ky(j))));
            ...
            eig_disr = eig_disr + TestEing(conj(H_eff(xi,-kx(i)+qx(iq)/2,-ky(j))),energy(:,i,j),band(:,:,i,j));
            if eig_disr > 10^(-5)
    ...
    for i = 1 : nx % to stay in the limit
        for j = 1 : ny
            for n = 1 : nbands
                H_eff_B(:,:,i,j,iq) = H_eff_B(:,:,i,j,iq) + energy_eff(n,i,j)*band_eff(:,n,i,j)*band_eff(:,n,i,j)';
            end
            [Vec,Val] = eigenshuffle(H_eff_B(:,:,i,j,iq));
            energy_eff_test(:,i,j) = Val(:);    % band, at which kx, at which ky;
            eig_disr = eig_disr + TestEing(H_eff_B(:,:,i,j,iq),energy_eff_test(:,i,j),Vec(:,:));
            if eig_disr > 10^(-5)
                disp('eigenality error')
            end
        end
...
J_xB       = zeros (nbands, nbands, nx, ny, q_size);

for iq = 1 : length(qx)
    for i = 3 : nx-2
        for j = 1 : ny
            J_xB(:,:,i,j,iq) = -H_eff_B(:,:,i+2,j,iq)+8*H_eff_B(:,:,i+1,j,iq)...
                             -8*H_eff_B(:,:,i-1,j,iq)+  H_eff_B(:,:,i-2,j,iq);
        end
    end
end
J_xB = J_xB/(12*(qx(2)-qx(1)));
% figure
% hold on
